  ...
  AV.Cloud.define("CsToPeerHistory", function(req, res) {
      ...
      var uri_list = _.map(plist, function(peer) {
          ...
          if (cus < peer) {
              ...
              return uri = 'https://cn.avoscloud.com/1.1/rtm/messages/logs' + '?convid=' + convid + '&limit=' + limit + '&timestamp=' + timestamp;
          } else {
              id_string = peer + ":" + cus;
              convid = crypto.createHash('md5').update(id_string).digest('hex');
              return uri = 'https://cn.avoscloud.com/1.1/rtm/messages/logs' + '?convid=' + convid + '&limit=' + limit + '&timestamp=' + timestamp;
  ...
  AV.Cloud.define("GetChatHistory", function(req, res) {
      ...
      convid = crypto.createHash('md5').update(id_string).digest('hex');
      console.log(convid);
      uri = 'https://cn.avoscloud.com/1.1/rtm/messages/logs' + '?convid=' + convid + '&limit=' + limit + '&timestamp=' + timestamp;
      console.log(uri);
      c
          .end(function(response) {
  ...
  var F = AV.Object.extend("_File");
  AV.Cloud.define("getFileCount", function(req,res) {
      var f = new AV.Query(F);
      f.count().then(function(count) {
          res.success(count)
      })
  });
